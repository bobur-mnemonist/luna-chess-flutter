...
class _ChessBoardScreenState extends State<ChessBoardScreen> {
  ...
  // The board FEN an engine move (Luna's turn) was requested for. Mirrors
  // _hintRequestedForFen: if Restart/Undo change the position again before
  // Stockfish answers a prior 'go', the bestmove that eventually arrives
  // belongs to a position that no longer exists and must be discarded
  // rather than applied to the current (different) game state — this was
  // the root cause of the board freezing / Luna's move silently not
  // landing when Restart or Undo were tapped in rapid succession.
  ...
  void _maybeTriggerEngineMove() {
    ...
    // one, leaving the board looking frozen.
    if (aiThinking) {
      engine.stdin = 'stop';
    }
