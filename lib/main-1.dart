...
// A tick sound for scroll-wheel pickers (Difficulty, Time) — uses the
// platform's built-in system click sound rather than reusing the move
// SFX at low volume, since the move sound was still recognizably "the
// same sound as making a chess move" even quieted down, which read as
// wrong/confusing in a settings picker. SystemSound needs no bundled
// audio asset and is instantly distinct from in-game SFX.
void _playWheelTick() {
...
class _ChessBoardScreenState extends State<ChessBoardScreen> {
  ...
  void _playMoveSound({bool isCapture = false}) {
    ...
    // Dispose once playback finishes — leaving these undisposed (the
    // previous behavior) accumulates native audio resources over a long
    // session, which is the likely cause of sounds becoming unreliable
    // later in a game, especially when two sounds fire close together
    // (e.g. a pawn promoting into check plays both a promotion move and
    // an immediate check sound).
    player.onPlayerComplete.listen((_) => player.dispose());
    player.play(AssetSource(asset, mimeType: 'audio/wav'), volume: 1.0);
  }
