        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              LunaPanel(mood: 'idle', line: lunaLine),
              const SizedBox(height: 24),
              const Text(
                'Difficulty',
...
class LunaPanel extends StatefulWidget {
  ...
  final String? line; // current speech bubble text, null = no bubble shown
  final bool voiceOn;
...
class _LunaPanelState extends State<LunaPanel> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    // Slow, subtle breathing scale loop — gives the still image a sense of
    // being alive without needing a drawn/frame-based animation asset.
    ...
    // Blink: a quick vertical squash, fired on an irregular timer so it
  ...
  void _scheduleNextBlink() {
    final randomDelay = 2500 + (DateTime.now().millisecondsSinceEpoch % 3000);
  ...
  // Mood affects a subtle rotation and color tint rather than swapping
  // images, since only one Luna image exists.
  double _moodTilt() {
    switch (widget.mood) {
      case 'happy':
        return -0.04;
  ...
  Color _moodTint() {
    switch (widget.mood) {
      case 'happy':
        return const Color(0x0088E0A0); // faint green wash
      case 'worried':
        return const Color(0x22E08080); // faint red wash
      case 'thinking':
        return const Color(0x1188AEE0); // faint blue wash
      default:
...
class _ChessBoardScreenState extends State<ChessBoardScreen> {
  ...
  void _applyEngineMove(String uciMove) {
    ...
    setState(() {
      ...
      aiThinking = false;
      _updateStatus();
      lunaLine = _pickLine(isCapture ? _engineCaptureLines : _engineMoveLines);
    });
  ...
  void _makeMove(String from, String to, {String promotion = 'q'}) {
    ...
    _playMoveSound(isCapture: isCapture);
    _updateStatus();
    lunaLine = _pickLine(isCapture ? _humanCaptureLines : _humanMoveLines);
