...
const _kBoardIs3D = 'board_is_3d';
const _kMenuSkillLevel = 'menu_skill_level';
const _kMenuPlayerColor = 'menu_player_color';
const _kMenuTimeControlMinutes = 'menu_time_control_minutes'; // -1 = untimed
...
// The player's name, collected once on first launch (before onboarding)
// so Luna can address them by name during gameplay. Empty/missing means
// no name was given — Luna falls back to not using a name at all rather
...
// The single non-consumable "unlock everything" product: Puzzles, Memory
// Training, and unlimited Luna commentary. One-time purchase, no
// subscription — matches Google Play Billing's non-consumable type
// (buyNonConsumable), which never expires and is restored via
// restorePurchases() on reinstall.
const String kPremiumProductId = 'luna_chess_premium';
const String _kIsPremium = 'is_premium';
// Each locked feature gets exactly one free trial use before requiring
// premium — these flip to true the first time a non-premium user
// finishes (or starts) a Puzzles/Memory Training session, and are
// checked alongside isPremium wherever those features are gated.
const String _kPuzzlesTrialUsed = 'puzzles_trial_used';
const String _kMemoryTrialUsed = 'memory_trial_used';
// Luna's free commentary lines: originally 10 per game (reset on every
// Restart/new game), changed per user request to 10 total across the
// whole app's lifetime — once spent, only a premium purchase brings her
// back, even after Restart. Persisted so it survives app restarts too.
const String _kLunaFreeLinesRemaining = 'luna_free_lines_remaining';

// DEVELOPER TESTING ONLY — set to true to force isPremium=true on this
// build, skipping the store entirely. Meant for the developer's own
// personal builds only (e.g. to test the unlocked experience before real
// Play Console in-app products exist) — NEVER set true in a build meant
// for actual users, since it silently gives everyone premium for free
// with no purchase. Leave false for any build that will be shared,
// side-loaded to a tester, or uploaded to Play Console.
const bool kForceUnlockPremiumForTesting = true;

// Wraps the in_app_purchase plugin for the app's one product. A
// ChangeNotifier so any screen can listen for "premium just unlocked"
// and rebuild (e.g. the menu removing a lock icon) without polling.
// Purchase state is cached in shared_preferences so the app doesn't need
// to re-verify with the store on every single screen open — only on
// launch (via restorePurchases) and right after a purchase completes.
class PremiumManager extends ChangeNotifier {
  ...
  Future<void> init() async {
    if (kForceUnlockPremiumForTesting) {
      isPremium = true;
      notifyListeners();
      return;
    }
    ...
    // Reconcile with the store in case a purchase completed outside this
    // install (new device, reinstall) — restorePurchases() re-delivers
    // any past non-consumable purchase through the same purchaseStream.
    if (!isPremium) {
  ...
  Future<void> buy() async {
    final p = product;
    if (p == null || purchasePending) return;
...
// A dedicated, reused AudioPlayer for the Settings wheel pickers' tick
// sound (Difficulty, Time). SystemSound.play(SystemSoundType.click) was
// tried first but is unreliable — it's silent whenever the device's
// "touch sounds" system setting is off, which is common. This reuses the
// existing move sound asset but played back at a raised pitch/rate so it
// reads as a distinct UI "tick" rather than literally the chess move
// sound, while still not requiring a new bundled audio file. Uses one
// long-lived player (created lazily, never disposed — Settings can be
// opened many times per app session) rather than a fresh AudioPlayer per
// tick, matching the fix applied to the in-game SFX after AudioPlayer
// churn was found to cause intermittent silent playback there too.
AudioPlayer? _wheelTickPlayer;
bool _wheelTickPlayerHasSource = false;

void _playWheelTick() {
  final player = _wheelTickPlayer ??= AudioPlayer()..setReleaseMode(ReleaseMode.stop);
  player.setPlaybackRate(1.8);
  if (!_wheelTickPlayerHasSource) {
    _wheelTickPlayerHasSource = true;
    player.play(AssetSource('audio/move-2.wav', mimeType: 'audio/wav'), volume: 0.7);
  } else {
...
// exact same thing every time the same event happens. When a userName is
// available, about a third of the time it's woven into the line (as a
// natural-sounding prefix or suffix) so Luna occasionally addresses the
// player by name without needing a hand-written name-aware variant of
// every single line in the game.
String _pickLine(List<String> options, {String? userName}) {
  final base = options[Random().nextInt(options.length)];
  if (userName == null || userName.isEmpty) return base;
  if (Random().nextDouble() > 0.35) return base;
  // Insert the name right after the first word (usually right after an
  // interjection like "Nice" or "Ooh") when that reads naturally, else
  // just prepend it — simple heuristic, not grammar-perfect but avoids
  // always bolting the name onto the same spot in every line.
  final commaIdx = base.indexOf(',');
  if (commaIdx > 0 && commaIdx < 12) {
    return '${base.substring(0, commaIdx)}, $userName${base.substring(commaIdx)}';
  }
...
// The paywall for Puzzles, Memory Training, and unlimited Luna
// commentary. Shown when a locked feature is tapped, or reachable
// directly. Listens to PremiumManager so it reflects purchase-in-flight
// and success/error states live.
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Luna Chess Premium', style: TextStyle(color: Colors.white)),
      ),
      body: SafeArea(
        child: ListenableBuilder(
          listenable: PremiumManager.instance,
          builder: (context, _) {
            final manager = PremiumManager.instance;
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        'Puzzles, Memory Training, and unlimited Luna commentary are all unlocked.',
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 24),
                  _PremiumFeatureRow(emoji: '🧩', text: 'Unlimited Puzzles'),
                  const SizedBox(height: 12),
                  _PremiumFeatureRow(emoji: '🧠', text: 'Memory Training'),
                  const SizedBox(height: 12),
                  _PremiumFeatureRow(emoji: '💬', text: "Unlimited commentary from Luna"),
                  const SizedBox(height: 32),
                  if (manager.purchaseError != null) ...[
                    Text(
                  ...
                  if (!manager.isAvailable)
                    ...
                    _MenuButton(
                      label: manager.purchasePending
                          ? 'Processing...'
                          : 'Get Premium — ${manager.product!.price}',
                      onPressed: manager.purchasePending ? () {} : manager.buy,
                    ),
                  const SizedBox(height: 12),
                  Center(
                    child: TextButton(
...
class _PremiumFeatureRow extends StatelessWidget {
  ...
  const _PremiumFeatureRow({required this.emoji, required this.text});
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  static const _pages = [
    _OnboardingPage(
      emoji: '♟️',
      imagePath: 'assets/images/luna_idle-1.png',
      title: 'Meet Luna',
      body:
          "Luna is your chess companion — she comments on your moves and reacts to captures and checks. Set her strength anywhere from 1 to 20 in Settings: low levels play casually and teach you as you go, higher levels bring out her full Stockfish-powered strength.",
    ),
    _OnboardingPage(
      emoji: '🧩',
      title: 'Puzzles',
...
class _MenuScreenState extends State<MenuScreen> {
  int selectedSkillLevel = 1;
  PlayerColor selectedColor = PlayerColor.white;
  TimeControl selectedTimeControl = const TimeControl.untimed();
  BoardTheme selectedBoardTheme = BoardTheme.blueGray;
  SavedGame? savedGame;
  bool checkedForSavedGame = false;
  bool puzzlesTrialUsed = false;
  bool memoryTrialUsed = false;
  ...
  // Loads every standing menu setting (difficulty, color, time control,
  // board theme) from shared_preferences. Previously only board theme was
  // persisted here — the other three were changed in SettingsScreen and
  // held only in this screen's in-memory state, so they silently reverted
  // to the hardcoded defaults every time the app was closed and reopened.
  Future<void> _loadMenuSettings() async {
    final prefs = await SharedPreferences.getInstance();
    final themeName = prefs.getString(_kBoardThemeName);
    final skillLevel = prefs.getInt(_kMenuSkillLevel);
    final colorName = prefs.getString(_kMenuPlayerColor);
    final timeMinutes = prefs.getInt(_kMenuTimeControlMinutes);
    final puzzlesUsed = prefs.getBool(_kPuzzlesTrialUsed) ?? false;
    final memoryUsed = prefs.getBool(_kMemoryTrialUsed) ?? false;
    if (!mounted) return;
    setState(() {
      selectedBoardTheme = BoardTheme.byName(themeName);
      if (skillLevel != null) selectedSkillLevel = skillLevel;
      if (colorName != null) {
        selectedColor = PlayerColor.values.firstWhere(
          (c) => c.name == colorName,
          orElse: () => PlayerColor.white,
        );
      }
      if (timeMinutes != null) {
        selectedTimeControl = timeMinutes < 0
            ? const TimeControl.untimed()
            : TimeControl.minutes(timeMinutes);
      }
      puzzlesTrialUsed = puzzlesUsed;
      memoryTrialUsed = memoryUsed;
    });
  }

  Future<void> _saveMenuSettings() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_kBoardThemeName, selectedBoardTheme.name);
    await prefs.setInt(_kMenuSkillLevel, selectedSkillLevel);
    await prefs.setString(_kMenuPlayerColor, selectedColor.name);
    await prefs.setInt(
      _kMenuTimeControlMinutes,
      selectedTimeControl.isUntimed ? -1 : (selectedTimeControl.minutes ?? 10),
    );
  }
  ...
  // Puzzles and Memory Training: non-premium users get exactly one free
  // trial session each before the paywall kicks in. Premium users always
  // pass straight through (isPremium check short-circuits first). The
  // trial flag is written the moment the feature is opened (not on
  // completion) so leaving mid-session still counts as "used" — this
  // matches how a one-time free look is usually understood and avoids
  // needing to plumb a "did they finish" signal back out of two very
  // different screens.
  Future<void> _openPuzzles() async {
    if (!PremiumManager.instance.isPremium) {
      if (puzzlesTrialUsed) {
        Navigator.of(context).push(_fadeRoute(const PremiumScreen()));
        return;
      }
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool(_kPuzzlesTrialUsed, true);
      if (!mounted) return;
      setState(() => puzzlesTrialUsed = true);
    }
  ...
  Future<void> _openMemoryTrain() async {
    if (!PremiumManager.instance.isPremium) {
      if (memoryTrialUsed) {
        Navigator.of(context).push(_fadeRoute(const PremiumScreen()));
        return;
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool(_kMemoryTrialUsed, true);
      if (!mounted) return;
      setState(() => memoryTrialUsed = true);
    }
    Navigator.of(context).push(
  ...
  void _openFamousGames() {
    Navigator.of(context).push(_fadeRoute(const FamousGamesListScreen()));
  }
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const _LunaMenuAvatar(),
                      const SizedBox(height: 44),
                      if (checkedForSavedGame && savedGame != null) ...[
                        _MenuButton(label: 'Continue Game', onPressed: _continueGame),
                        const SizedBox(height: 18),
                      ],
                      _MenuButton(label: 'Start New Game', onPressed: _startGame),
                      const SizedBox(height: 18),
                      ListenableBuilder(
                        listenable: PremiumManager.instance,
                        builder: (context, _) {
                          final premium = PremiumManager.instance.isPremium;
                          // Each feature shows its own state: no badge at
                          // all once premium, a small "free trial" hint
                          // before the one free use is spent, and a lock
                          // once it has been.
                          String puzzlesLabel = 'Puzzles';
                          if (!premium) {
                            puzzlesLabel = puzzlesTrialUsed
                                ? 'Puzzles 🔒'
                                : 'Puzzles (1 free try)';
                          }
                          String memoryLabel = 'Memory Training';
                          if (!premium) {
                            memoryLabel = memoryTrialUsed
                                ? 'Memory Training 🔒'
                                : 'Memory Training (1 free try)';
                          }
                          return Column(
                            children: [
                              _MenuButton(
                                label: puzzlesLabel,
                                onPressed: _openPuzzles,
                              ),
                              const SizedBox(height: 18),
                              _MenuButton(
                                label: memoryLabel,
                                onPressed: _openMemoryTrain,
                              ),
                            ],
                          );
                        },
                      ...
                      const SizedBox(height: 18),
                      _MenuButton(label: 'Famous Games', onPressed: _openFamousGames),
                    ],
                  ),
                ),
              ),
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  // Loaded/saved directly against shared_preferences here (not passed in
  // via widget/_SettingsResult like the other settings) because
  // ChessBoardScreen already reads this same key independently in its own
  // initState — same pattern voiceOn follows. Keeping it out of
  // _SettingsResult avoids widening that class for a value neither the
  // menu screen nor _saveMenuSettings otherwise needs to know about.
  bool boardIs3D = false;
  ...
  Future<void> _loadName() async {
    final prefs = await SharedPreferences.getInstance();
    _nameController.text = prefs.getString(_kUserName) ?? '';
  }
  ...
  Future<void> _loadBoardIs3D() async {
    final prefs = await SharedPreferences.getInstance();
    final is3D = prefs.getBool(_kBoardIs3D) ?? false;
    if (mounted) setState(() => boardIs3D = is3D);
  }

  Future<void> _setBoardIs3D(bool value) async {
    setState(() => boardIs3D = value);
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_kBoardIs3D, value);
  }
                  ...
                  Padding(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: 'Not set — Luna won\'t use a name',
                        hintStyle: const TextStyle(color: Colors.white38, fontSize: 13),
                        filled: true,
                        fillColor: kPanelColor,
                    ...
                    children: [
                      Padding(
                        ...
                        child: ChoiceChip(
                          label: const Text('2D'),
                          selected: !boardIs3D,
                          onSelected: (_) => _setBoardIs3D(false),
                          selectedColor: kSelectedSquare,
                          backgroundColor: kPanelColor,
                          labelStyle: TextStyle(
                            color: !boardIs3D ? Colors.white : Colors.white70,
                            fontWeight:
                                !boardIs3D ? FontWeight.bold : FontWeight.normal,
                          ),
                        ),
                      ),
                      Padding(
                        ...
                        child: ChoiceChip(
                          label: const Text('3D'),
                          selected: boardIs3D,
                          onSelected: (_) => _setBoardIs3D(true),
                          selectedColor: kSelectedSquare,
                          backgroundColor: kPanelColor,
                          labelStyle: TextStyle(
                            color: boardIs3D ? Colors.white : Colors.white70,
              ...
              RotatedBox(
                ...
                child: ListWheelScrollView.useDelegate(
                  ...
                  onSelectedItemChanged: (index) {
                    // Haptic + a quiet audible tick per wheel step —
                    // haptic alone can go unnoticed on devices with
                    // vibration disabled, so both together match the
                    // feel of a physical scroll wheel/dial reliably.
...
class ChessBoardScreen extends StatefulWidget {
  ...
  const ChessBoardScreen({
    ...
    this.timeControl = const TimeControl.untimed(),
...
class _ChessBoardScreenState extends State<ChessBoardScreen> {
  ...
  bool voiceOn = true;
  // 2D/3D board view toggle. Loaded from shared_preferences in initState
  // (see _loadBoardViewPreference) and persisted whenever Settings changes
  // it. Purely visual — a Transform.rotate/scale wrapper around the same
  // GridView.builder board, not a separate 3D rendering engine, so it
  // doesn't affect move logic, hint/last-move highlighting, or anything
  // else that references board squares by index.
  bool boardIs3D = false;
  int? hintFromIndex;
  int? hintToIndex;
  ...
  // One AudioPlayer per SFX type, created once and reused for the whole
  // screen's lifetime instead of a fresh disposable AudioPlayer per move.
  // Creating a brand new native player for every move was the likely
  // cause of intermittently silent SFX (especially check, which often
  // fires right alongside a capture/promotion sound) — audioplayers'
  // underlying native player can drop playback when a prior instance's
  // teardown hasn't finished by the time a new one starts. Reusing a
  // small fixed pool and calling stop()+play() on each avoids that
  // churn entirely.
  late final AudioPlayer _moveSoundPlayer;
  late final AudioPlayer _captureSoundPlayer;
  late final AudioPlayer _checkSoundPlayer;
  String? userName;
  // Free tier gets Luna's commentary for 10 lines TOTAL across the app's
  // entire lifetime (not per game) — once spent, she stays quiet on every
  // future game until premium is purchased. Loaded from shared_preferences
  // in initState and decremented/persisted in _setLunaLine. Puzzles/Memory
  // Training are fully gated at the menu instead — this one is a soft
  // taste-then-upsell limit rather than an outright block, since
  // commentary is ambient rather than a discrete mode to lock.
  int _lunaFreeLinesRemaining = 10;
  // True once the free-lines limit is hit and Luna's bubble is showing
  // the upsell message instead of real commentary. While true, the
  // avatar's mood/image is frozen (see _lunaMood) rather than continuing
  // to react to check/checkmate/thinking — those reactions are tied to
  // move commentary, and swapping the avatar image right as the upsell
  // appears read as an unrelated, distracting change rather than Luna
  // actually reacting to something.
  bool _showingPremiumUpsell = false;
  ...
  // The board FEN an engine move (Luna's turn) was requested for. If
  // Restart/Undo change the position again before Stockfish answers a
  // prior 'go', the bestmove that eventually arrives belongs to a
  // position that no longer exists and must be discarded rather than
  // applied to the current (different) game state.
  String? _engineMoveRequestedForFen;
  // Counts 'stop' commands sent to cancel an in-flight engine-move
  // request that hasn't been superseded yet. Sending 'stop' itself
  // produces a bestmove reply from Stockfish (UCI semantics), which must
  // never be treated as a real move or allowed to touch aiThinking — a
  // genuine new request may already be in flight waiting for its own
  // answer. Every bestmove that arrives while this is > 0 is consumed as
  // one cancellation's reply (decrementing the counter) instead; this is
  // what root-caused the board freezing / Luna's move silently not
  // landing when Restart or Undo were tapped in rapid succession — the
  // earlier FEN-only check couldn't tell a stop's own reply apart from a
  // genuine new request's reply, and was clearing aiThinking on the
  // stop's reply even though a fresh request was already awaiting one.
  int _pendingEngineMoveCancellations = 0;
  ...
  // Central choke point for setting Luna's speech bubble text — every
  // lunaLine assignment in this screen goes through this instead of
  // calling _pickLine directly, so the free-tier line limit is enforced
  // in one place rather than needing a check at every call site. The
  // limit is 10 lines total across the app's whole lifetime (not per
  // game) — once spent, Luna goes quiet on every future game (bubble
  // shows a small "Premium" upsell) until premium is purchased.
  void _setLunaLine(List<String> options) {
    if (!PremiumManager.instance.isPremium) {
      if (_lunaFreeLinesRemaining <= 0) {
        lunaLine = "💬 Premium for more from Luna";
        _showingPremiumUpsell = true;
        return;
      }
      _lunaFreeLinesRemaining--;
      _saveLunaFreeLines(_lunaFreeLinesRemaining);
    }
    _showingPremiumUpsell = false;
    lunaLine = _pickLine(options, userName: userName);
  }

  // Fire-and-forget persistence for the global free-lines counter —
  // called on every decrement so the count survives Restart, leaving the
  // game, and even the app being closed and reopened.
  void _saveLunaFreeLines(int remaining) {
    SharedPreferences.getInstance().then(
      (prefs) => prefs.setInt(_kLunaFreeLinesRemaining, remaining),
    );
  }
  ...
  void initState() {
    super.initState();
    // Three reused AudioPlayer instances (one per SFX type) instead of a
    // fresh one per move — see the field comments above for why. Preload
    // each with its asset immediately so the very first play() call
    // doesn't have to do disk I/O before sound comes out.
    _moveSoundPlayer = AudioPlayer()..setReleaseMode(ReleaseMode.stop);
    _captureSoundPlayer = AudioPlayer()..setReleaseMode(ReleaseMode.stop);
    _checkSoundPlayer = AudioPlayer()..setReleaseMode(ReleaseMode.stop);
    _moveSoundPlayer.setSourceAsset('audio/move-2.wav');
    _captureSoundPlayer.setSourceAsset('audio/capture-2.wav');
    _checkSoundPlayer.setSourceAsset('audio/check-2.wav');
  ...
  Future<void> _loadLunaFreeLines() async {
    final prefs = await SharedPreferences.getInstance();
    final remaining = prefs.getInt(_kLunaFreeLinesRemaining) ?? 10;
    if (mounted) setState(() => _lunaFreeLinesRemaining = remaining);
  }

  Future<void> _loadBoardViewPreference() async {
    final prefs = await SharedPreferences.getInstance();
    final is3D = prefs.getBool(_kBoardIs3D) ?? false;
    if (mounted) setState(() => boardIs3D = is3D);
  }
  ...
  void _onEngineOutput(String line) {
    if (line.startsWith('bestmove')) {
      ...
      if (parts.length >= 2) {
        ...
        if (_pendingEngineMoveCancellations > 0) {
          // This bestmove is the reply to one of our own 'stop' commands,
          // not a genuine move request — consume it silently and don't
          // let it touch aiThinking, since a fresh request may already be
          // in flight waiting for its own separate reply.
          _pendingEngineMoveCancellations--;
          return;
        }
        if (_awaitingHint) {
          _awaitingHint = false;
  ...
  void _applyEngineMove(String uciMove) {
    // Safety net: if Restart/Undo changed the position again after this
    // move was requested but before Stockfish answered, this bestmove
    // belongs to a position that no longer exists — applying it would
    // silently corrupt the current game state (the freeze/stuck-board
    // symptom). Discard it and let the position's own fresh
    // _maybeTriggerEngineMove (already called by Restart/Undo) handle it.
    final stale = _engineMoveRequestedForFen != null &&
        _engineMoveRequestedForFen != game.fen;
    _engineMoveRequestedForFen = null;
    if (stale) {
    ...
    setState(() {
      ...
      aiThinking = false;
      _updateStatus();
      _setLunaLine(_lineForEngineMove(from: from, to: to, isCapture: isCapture));
  ...
  void _maybeTriggerEngineMove() {
    ...
    // If a previous engine-move request never resolved (e.g. Restart or
    // Undo was tapped again before Stockfish answered), tell it to stop
    // searching before starting a new one — otherwise the two 'go'
    // commands queue up and the eventual bestmove may correspond to
    // whichever position was current when it was issued, not the current
    // one. Note this 'stop' itself produces a bestmove reply — that reply
    // is tagged with the OLD request id and gets silently dropped by
    // _onEngineOutput/_applyEngineMove once the new id below is in place.
    if (aiThinking) {
      engine.stdin = 'stop';
      _pendingEngineMoveCancellations++;
    }
  ...
  void _makeMove(String from, String to, {String promotion = 'q'}) {
    ...
    _playMoveSound(isCapture: isCapture);
    _updateStatus();
    _setLunaLine(_lineForHumanMove(from: from, to: to, isCapture: isCapture));
  ...
  void _playMoveSound({bool isCapture = false}) {
    // Check takes priority (most eventful), then capture, then a plain
    // move — same as before, just routed to the matching pooled player
    // instead of building a brand new AudioPlayer for every call.
    final AudioPlayer player;
    if (game.in_check) {
      player = _checkSoundPlayer;
    } else if (isCapture) {
      player = _captureSoundPlayer;
    } else {
      player = _moveSoundPlayer;
    }
    // stop() first (harmless if already stopped) rewinds a still-playing
    // clip back to the start before resume(), so back-to-back triggers of
    // the same sound (e.g. two captures in quick succession) restart
    // cleanly instead of the second call being ignored because the
    // player thinks it's already "playing".
    player.stop().then((_) => player.resume());
  }
  ...
  void _updateStatus() {
    if (game.in_checkmate) {
      ...
      _clearSavedGame();
      _setLunaLine(winner == 'White' ? _humanWinLines : _engineWinLines);
      final winnerColor =
          winner == 'White' ? chess_lib.Color.WHITE : chess_lib.Color.BLACK;
      _recordGameResult(winnerColor == humanSide ? 'win' : 'loss');
  ...
  void _restart() {
    // If an engine-move request from before this Restart is still
    // in-flight (rapid repeated taps), stop it now — resetting
    // aiThinking to false below would otherwise hide that fact from
    // _maybeTriggerEngineMove's own in-flight check, letting two 'go'
    // commands queue up on Stockfish.
    if (aiThinking) {
  ...
  void _undo() {
    ...
    // Same in-flight-request guard as _restart: stop Stockfish if it's
    // still working on a move for the pre-undo position before wiping
    // aiThinking, so a late bestmove for that now-gone position can't
    // land on the post-undo board.
    if (aiThinking) {
      stockfish?.stdin = 'stop';
      _pendingEngineMoveCancellations++;
    }
  ...
  String? _lastRealMood;

  String get _lunaMood {
    if (_showingPremiumUpsell) {
      // Freeze on whatever mood was showing right before the upsell
      // appeared, instead of snapping to a default or continuing to
      // react to game state that the upsell message isn't about.
      return _lastRealMood ?? 'idle';
    }
    String mood;
    if (game.in_checkmate) {
      mood = game.turn == humanSide ? 'happy' : 'worried';
    } else if (game.in_check && game.turn != humanSide) {
      mood = 'happy';
    } else if (game.in_check) {
      mood = 'worried';
    } else if (aiThinking) {
  ...
  // Wraps the board in a light perspective effect when boardIs3D is on —
  // a 3D-ish "tilted on a table" look via a perspective Matrix4 transform
  // plus a drop shadow, not a real 3D rendering engine or separate asset
  // set. The underlying GridView.builder board (squares, pieces,
  // highlights, gestures) is completely unchanged either way, so 2D/3D is
  // purely a rendering choice made here, not two different board
  // implementations to keep in sync.
  Widget _buildBoardWithOptional3D({required Widget child}) {
    if (!boardIs3D) return child;
    final transform = Matrix4.identity()
      ..setEntry(3, 2, 0.0016) // perspective depth
      ..rotateX(0.35); // tilt the far edge away from the viewer
    return Container(
      decoration: BoxDecoration(
        boxShadow: [
          BoxShadow(
...
// A single famous game: metadata for the intro card plus its full move
// list in UCI form (e.g. "e2e4"). All games here were chosen specifically
// for being well short enough to hand-verify every move structurally
// (piece present, correct color to move, castling/en passant handled)
// rather than risking a transcription error in a very long grandmaster
// game — accuracy matters more than including every classic.
class FamousGame {
  final String title;
...
final List<FamousGame> kFamousGames = [
  const FamousGame(
    title: 'The Opera Game',
    players: 'Paul Morphy vs. Duke Karl / Count Isouard',
    event: 'Paris Opera House, 1858',
    description:
        "Played during an opera performance, Morphy sacrifices material for rapid development and finishes with a queen sacrifice leading to checkmate. A textbook example of piece activity beating material.",
    moves: [
  ...
  const FamousGame(
    ...
    event: 'Berlin, 1852',
    description:
        "One of chess's most celebrated attacking games. Anderssen builds a storm of sacrifices — a rook, both bishops, and finally the queen — to force checkmate with his remaining minor pieces.",
    moves: [
      ...
      "a4d7","e8d7","d3f5","d7e8","f5d7","e8f8","d7e7",
    ],
  ),
  const FamousGame(
    ...
    description:
        "Widely regarded as one of the greatest games ever played. Kasparov sends his king on a walk into the open board while sacrificing a rook and exchange, calculating over 15 moves deep to a forced win.",
    moves: [
  ...
  const FamousGame(
    title: "Legal's Mate",
    players: 'Kermur Sire de Légal (attributed), 18th century',
    event: 'A famous trap, not a formal tournament game',
    description:
        "A classic opening trap: White lets Black capture the queen, but the resulting position lets White's remaining pieces deliver checkmate anyway — the queen was worth less than the mating attack.",
    moves: [
      "e2e4","e7e5","f1c4","d7d6","g1f3","c8g4","b1c3","g8h6","f3e5",
      "g4d1","c4f7","e8e7","c3d5",
    ],
  ),
  const FamousGame(
    title: "Scholar's Mate",
    players: 'A well-known beginner trap',
    event: 'Educational pattern',
    description:
        "The fastest common checkmate pattern: White's queen and bishop team up on f7 in just four moves. A good reminder to defend that square early against unprepared opponents — and to watch for it yourself.",
  ...
  const FamousGame(
    ...
    event: 'Educational pattern',
    description:
        "The shortest possible checkmate in chess — just two moves each. It requires White to badly weaken their own king, but it's worth knowing exactly what to avoid.",
    moves: ["f2f3","e7e5","g2g4","d8h4"],
  ),
];

class FamousGamesListScreen extends StatelessWidget {
  const FamousGamesListScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        iconTheme: const IconThemeData(color: Colors.white70),
        title: const Text('Famous Games', style: TextStyle(color: Colors.white)),
      ),
      body: SafeArea(
        child: ListView.separated(
          padding: const EdgeInsets.all(16),
          itemCount: kFamousGames.length,
          separatorBuilder: (context, index) => const SizedBox(height: 12),
          itemBuilder: (context, index) {
            final game = kFamousGames[index];
            return InkWell(
              borderRadius: BorderRadius.circular(12),
              onTap: () => Navigator.of(context).push(
                _fadeRoute(FamousGameViewScreen(game: game)),
              ),
              child: Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
...
// Shows a famous game's context (who played it, when, why it matters)
// first, then lets the user step through it move by move on a board with
// the same eval/quality-annotation treatment as the regular Analysis
// screen. Reuses AnalysisScreen's engine/eval logic by simply computing
// the fenHistory up front from the game's UCI move list and handing off.
class FamousGameViewScreen extends StatefulWidget {
  final FamousGame game;

  const FamousGameViewScreen({super.key, required this.game});

  @override
  State<FamousGameViewScreen> createState() => _FamousGameViewScreenState();
}

class _FamousGameViewScreenState extends State<FamousGameViewScreen> {
  bool _showingIntro = true;

  List<String> _buildFenHistory() {
    final game = chess_lib.Chess();
    final history = <String>[game.fen];
    for (final uci in widget.game.moves) {
  ...
  Widget build(BuildContext context) {
    if (_showingIntro) {
      // The context-first intro: who played it and what game/event it
      // was, explained before the user ever sees a move — this was the
      // whole point of the request, so it's not skippable, just a single
      // tap through to the board.
      return Scaffold(
        appBar: AppBar(
          backgroundColor: Colors.transparent,
          elevation: 0,
    ...
    return AnalysisScreen(fenHistory: _buildFenHistory());
  }
}
...
// A large, static Luna avatar for the menu screen — just the idle image,
// no mood switching, no speech bubble, no TTS. Kept entirely separate
// from LunaPanel (used during gameplay) so this doesn't touch or risk
// breaking any of that widget's mood/commentary logic.
class _LunaMenuAvatar extends StatelessWidget {
  const _LunaMenuAvatar();
...
// A larger button variant for the menu screen's primary actions
// (Start/Continue/Puzzles/Memory Training), which need more visual
// weight than the compact in-game action row buttons (_ActionButton) —
// kept separate rather than resizing _ActionButton globally, since that
// one is shared by dense rows like Undo/Restart/Hint where a bigger
// button would crowd the row.
