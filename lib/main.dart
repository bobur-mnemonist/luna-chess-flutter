...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _proceedAfterDelay() async {
    ...
    final hasName = (prefs.getString(_kUserName) ?? '').isNotEmpty;
    if (!mounted) return;
    // A truly fresh install sees the app intro (Onboarding) first, then
    // is asked for a name once that's done. Once both are on record,
    // later launches skip straight past both to the menu.
    Widget next;
    if (!hasSeenOnboarding) {
      next = const OnboardingScreen();
    } else if (!hasName) {
      next = const NameEntryScreen();
    } else {
...
// First-launch-only screen (shown once, before onboarding) that asks the
// player's name so Luna can address them by it during gameplay. Skippable
// — a blank/skipped entry just means Luna never uses a name, rather than
// falling back to a placeholder like "Player".
...
class _NameEntryScreenState extends State<NameEntryScreen> {
  ...
  Future<void> _proceed() async {
    ...
    // Always write the key (even if empty) so _proceedAfterDelay's
    // "hasName" check on future launches reads a definitive answer
    // rather than re-showing this screen because the key was never set.
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "Luna will use it while you play.",
                textAlign: TextAlign.center,
              ...
              _MenuButton(label: 'Continue', onPressed: _proceed),
              const SizedBox(height: 12),
              TextButton(
                onPressed: _proceed,
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Future<void> _finish() async {
    ...
    if (!mounted) return;
    Navigator.of(context).pushReplacement(_fadeRoute(const NameEntryScreen()));
  }
              ...
              RotatedBox(
                ...
                child: ListWheelScrollView.useDelegate(
                  ...
                  onSelectedItemChanged: (index) {
                    // A light haptic tick per wheel step, matching the
                    // feel of a physical scroll wheel/dial — cheaper and
                    // more reliable than bundling+playing an audio clip
                    // for every single scroll tick.
