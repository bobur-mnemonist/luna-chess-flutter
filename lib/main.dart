...
final AudioPlayer _wheelTickPlayer = AudioPlayer()..setReleaseMode(ReleaseMode.stop);
bool _wheelTickPlayerReady = false;

// Preloads the tick sound source once, ahead of any actual tick. Called
// from main() before runApp so the player is already loaded by the time
// the user reaches Settings. This removes the previous lazy-load-on-
// first-tick race: loading the source is itself async, and if a second
// tick fired before that first load finished, stop()/resume() would run
// against a player that wasn't ready yet, causing the tick to silently
// drop — which matched the "sometimes plays, sometimes doesn't" report.
Future<void> _initWheelTickPlayer() async {
  await _wheelTickPlayer.setPlaybackRate(1.8);
  await _wheelTickPlayer.setVolume(0.7);
  await _wheelTickPlayer.setSourceAsset('assets/audio/move-2.wav');
  _wheelTickPlayerReady = true;
}

void _playWheelTick() {
  if (!_wheelTickPlayerReady) return; // still loading; skip rather than race
  _wheelTickPlayer.stop().then((_) => _wheelTickPlayer.resume());
}
...
class _MenuScreenState extends State<MenuScreen> {
  ...
  void _openPuzzles() {
    // Premium gate temporarily disabled — open to everyone for now.
    // (PremiumManager/PremiumScreen kept intact; re-add the check below
    // to re-enable: if (!PremiumManager.instance.isPremium) { ... }.)
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 18),
                      // Premium gate temporarily disabled — both buttons
                      // shown unlocked for everyone. ListenableBuilder kept
                      // (rather than removed) so re-enabling the lock later
                      // is a one-line revert: swap `false` back to
                      ...
                      ListenableBuilder(
                        ...
                        builder: (context, _) {
                          ...
                          return Column(
                            children: [
                              _MenuButton(
                                label: locked ? 'Puzzles 🔒' : 'Puzzles',
                                onPressed: _openPuzzles,
                              ),
                              const SizedBox(height: 18),
                              _MenuButton(
                                label: locked ? 'Memory Training 🔒' : 'Memory Training',
...
class _ChessBoardScreenState extends State<ChessBoardScreen> {
  ...
  void initState() {
    ...
    _moveSoundPlayer.setSourceAsset('assets/audio/move-2.wav');
    _captureSoundPlayer.setSourceAsset('assets/audio/capture-2.wav');
    _checkSoundPlayer.setSourceAsset('assets/audio/check-2.wav');
