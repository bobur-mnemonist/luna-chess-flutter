...
class _MenuScreenState extends State<MenuScreen> {
  ...
  void _openMemoryTrain() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => MemoryTrainScreen(boardTheme: selectedBoardTheme),
      ),
    );
  }
...
class _PuzzleScreenState extends State<PuzzleScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // still has to work out where it goes.
    final hintFromIndex = (phase == _PuzzlePhase.solving && hintRevealed)
        ? _indexForSquareName(solutionFrom)
        : null;
                  ...
                  child: Padding(
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isSolutionSquare = _squareIndex(row, col) == solutionFromIndex ||
                            _squareIndex(row, col) == solutionToIndex;
...
class _MemoryTrainScreenState extends State<MemoryTrainScreen> {
  ...
  static const int studySeconds = 12;
  int secondsLeft = studySeconds;
  Timer? _studyTimer;
  ...
  void _generateNewRound() {
    ...
    setState(() {
      ...
      // A bit shorter than puzzle setup so positions stay memorable —
      // fewer pieces to track, not an overwhelming full-board jumble.
      _setupMovesRemaining = 8 + _random.nextInt(9);
    });
    _playNextSetupMove();
  }
