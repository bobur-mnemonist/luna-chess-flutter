...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _proceedAfterDelay() async {
    ...
    if (!mounted) return;
    // A truly fresh install (never asked for a name) goes to the name
    // entry screen first, then onboarding. Once a name is on record,
    // later launches skip straight past both to the menu as before.
    Widget next;
    if (!hasName) {
