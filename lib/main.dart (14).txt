...
class _ChessBoardScreenState extends State<ChessBoardScreen> {
  ...
  void initState() {
    ...
    AudioCache.instance.loadAll(['audio/move.wav', 'audio/capture.wav', 'audio/check.wav']);
  ...
  void _playMoveSound({bool isCapture = false}) {
    ...
    // A fresh AudioPlayer per call, at max volume, set to release
    // immediately after playing. Reusing one long-lived AudioPlayer across
    // many rapid plays is a known source of silent/stuck playback on some
    // Android OEM audio stacks — a disposable player avoids that class of
    // issue entirely.
    final player = AudioPlayer();
    player.setReleaseMode(ReleaseMode.release);
    player.play(AssetSource(asset), volume: 1.0).then((_) {
      if (mounted) setState(() => statusText = 'Played: $asset');
    }).catchError((error) {
      if (mounted) setState(() => statusText = 'Audio error: $error');
    });
  }
