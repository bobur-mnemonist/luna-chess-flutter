...
class _ChessBoardScreenState extends State<ChessBoardScreen> {
  ...
  List<String> fenHistory = [chess_lib.Chess().fen];
  ...
  void _playMoveSound() {
    if (!soundOn) return;
    // System click sound as a lightweight stand-in for a dedicated move SFX
    // (no bundled audio asset yet, so no extra package/asset needed).
  ...
  void _backToMenu() {
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
          'Going to the menu will end this game. Progress will not be saved.',
